...
function placeXOrO(squareNumber) {
    ...
    //to see if it contains the square number clicked to.
    if (!selectedSquares.some(element => element.includes(squareNumber))) {
        ...
        if (activePlayer === 'X') {
            ...
            select.style.backgroundImage = 'url("./images/waffles.jfif")';
            //Active player must only be 'X' or 'O' so, if not 'X' it must be 'O'
        } else {
            //If activePlayer is equal to 'O', the o.png is placed in HTML.
            select.style.backgroundImage = 'url("./images/burger.jfif")';
        }
        //squareNumber and activePlayer are concatenated together and added to array.
        selectedSquares.push (squareNumber + activePlayer);
        //This calls a function to check for any win conditions.
        checkWinConditions();
        //This condition is for changing the active players.
        if (activePlayer === 'X') {
...
function resetGame () {
    ...
    for (let i = 0; i < 9; i++) {
        //This variable gets the html element from i.
